...
* *Magical developer experience*. Signup for an account and send your first SMS in five minutes with five lines of code in the language you already know. 
* *Exceptional documentation.* Copy-pastable code (that *works*) to support devs on their journey from quickstart, to production, to scale. 
* *Technical blog content* to generate awareness and signups via organic and developer-social channels. 
...
AI and the end of ZIRP is changing how devtool companies reach and serve developers. 

This site is an attempt to document what effective devrel might look like in the age of AI. I won't pretend to have all the answers, but I have lots of questions and a few smart friends. 

# Questions about devrel in 2025 
...
We wrote different content to target Organic traffic vs. Social traffic. A post called "How to Send SMS with Ruby" performs well in Google, whereas "How I taught my dog to text selfies" performs well on Hacker News. The former drove signups, the latter drove awareness. 

I conceptualized this as "content a developer reads with an IDE open" and "content a developer reads on the toilet." 

* Re: "content a developer reads with an IDE open." Developers increasingly solve those problems without leaving their IDE. Is there still value in shipping the "how to send sms in ruby" blog post? 
  
* Let's say for sake of argument,"search is dead, de-prioritize the blog," BUT you still need content to get included in the training data. The more content in the training data, the more likely the LLM is to suggest your tool. (Is this actually true, or does RLHF and fine-tuning counteract this?) 
  
* If that's true, we're incentivized to fill the internet with LLM generated technical slop to feed the training data. Feels like SEO in 2002. How will model providers curate training data to avoid this race to the bottom? 

* Are we all going to develop slop fatigue that makes a corporate blogs feel kind of gross? 
...
* Will there be an Google AdWords equivalent for LLMs where (devtool) companies pay for preferential treatment? Will OpenAI let be a "verified source" of training data for a "small fee", so that Twilio is recommended first when a developer asks how to send an sms in ruby? 
...
This is a project by Greg Baugues. The questions and thoughts here were cultivated through conversations with many friends, especially: Ricky Robinett, Andrew Baker, Matt Makai. 
